...
static CliOptions? ParseArgs(string[] args)
{
    ...
    if (mode != "analyze" && mode != "migrate" && mode != "verify" && mode != "verify-project" && mode != "doctor" && mode != "explain-todo" && mode != "smoke-plan" && mode != "migration-board" && mode != "profile-match" && mode != "config-validate" && mode != "config-diff" && mode != "guard" && mode != "propose" && mode != "discover-target" && mode != "index-pom" && mode != "orchestrate" && mode != "scaffold" && mode != "bootstrap-project")
    {
        Console.Error.WriteLine($"Invalid mode: {mode}. Use: analyze|migrate|verify|verify-project|doctor|explain-todo|smoke-plan|migration-board|profile-match|config-validate|config-diff|guard|propose|discover-target|index-pom|orchestrate|scaffold|bootstrap-project");
        return null;
    }
...
Options:
    --mode <mode>                 Operation mode (required)
                                    analyze|migrate|verify|verify-project|doctor|explain-todo|smoke-plan|migration-board|profile-match|config-validate|config-diff|guard|propose|discover-target|index-pom|orchestrate|scaffold|bootstrap-project

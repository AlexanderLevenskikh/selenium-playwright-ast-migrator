...
internal static class CliCommandCatalog
{
    ...
    static readonly CliCommandInfo[] Commands =
    {
        ...
        StableCommand("migration", "migration", false, false,
            ...
            "Direct command family: `selenium-pw-migrator migration inventory|cluster|tune-wave-plan|plan|plan show|run-wave|validate-wave|validation-plan|record-validation|checkpoint-wave|resume-wave|build-review-bundle|check-progress|perf-report`. `tune-wave-plan` and planning are read-only and invoke no agents; `plan --wave-profile auto` evaluates deterministic batching profiles, charges repeated tests from one file at marginal complexity, and writes wave-tuning evidence. `run-wave` materializes one selected wave as an immutable bounded project-local workspace with fast/standard/audit execution policy and run context; incremental commands compute changed-file impact, reuse only exact-input PASS validation, checkpoint/resume work, and prepare reviewer evidence. `validate-wave` rejects drift, `check-progress` stops repeated loops, and `perf-report` prints phase timings. In OpenCode, prefer `/supervised-task waves`.",

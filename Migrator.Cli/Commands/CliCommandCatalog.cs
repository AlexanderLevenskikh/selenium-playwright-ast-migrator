...
internal static class CliCommandCatalog
{
    ...
    static readonly CliCommandInfo[] Commands =
    {
        ...
        StableCommand("pilot", "pilot", true, true,
            "Select a representative bounded calibration slice before the first full-project run.",
        ...
        StableCommand("lab", "artifacts/lab", false, false,
            ...
            "Direct command family: `selenium-pw-migrator lab validate|list|app serve`. Blocks 1-2 validate lab-scenario/v1 contracts, verify fixed fixture hashes, list ready scenarios, and host deterministic LabApp pages. Later blocks add run, replay, diff, and minimize without creating a separate product binary.",
            ...
            "selenium-pw-migrator lab list --corpus ./corpus/stable/vertical-slice --state ready"),
        ...
        StableCommand("run", "migration", true, true,
            "Run the standard full-project migration pipeline.",
            "Runs analyze, generate, verify, and mapping-proposal stages in one linear pass. The command writes an ordinary run directory and never creates batch plans, partition workspaces, acceptance receipts, or partition-specific state. Use pilot first when you want a small representative calibration sample, then run this command for the complete source scope.",
            "Source Selenium test file or directory.",
            "selenium-pw-migrator run --input ./OldTests --config ./adapter-config.json --out migration/run-001",
            "selenium-pw-migrator pilot --input ./OldTests --config ./adapter-config.json --out migration/pilot"),
        StableCommand("config-merge", "config merge-deltas", false, false,
            "Merge project-local run config deltas into a reviewable adapter-config candidate.",

...
internal static class CliCommandCatalog
{
    ...
    static readonly CliCommandInfo[] Commands =
    {
        ...
        StableCommand("runbook", "runbook", true, true,
            ...
            "Reads source/project signals, optional config layers, source/target capability metadata, and writes runbook.md/json with pilot scope, first command chain, risk map, artifacts to collect, and acceptance checklist. Does not modify source files.",
            "Source Selenium test file or project directory.",
            "selenium-pw-migrator runbook --input ./OldTests --target dotnet --target-test-framework xunit --out runbook",
        ...
        ExperimentalCommand("runtime-classify", "runtime-classify", true, true,
            "Classify Playwright/NUnit/xUnit runtime failures and trace evidence after smoke runs.",
            "Groups common runtime failures such as locator-not-found, strict-mode-violation, timeout-wait-state, assertion-mismatch, navigation-route-missing, auth/session-not-ready, test-data-missing, modal/dialog-state, frame/shadow-dom, and environment/flaky-infra. Also indexes trace zips, screenshots, videos, console/network artifacts, and generated/source context links when available.",

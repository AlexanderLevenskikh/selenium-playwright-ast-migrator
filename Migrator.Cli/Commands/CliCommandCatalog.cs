...
internal static class CliCommandCatalog
{
    ...
    static readonly CliCommandInfo[] Commands =
    {
        ...
        StableCommand("migration", "migration", false, false,
            ...
            "Direct command family: `selenium-pw-migrator migration inventory|cluster|tune-wave-plan|plan|plan show|run-wave|validate-wave|measure-wave|record-wave-decision|record-wave-remediation|accept-wave|check-wave-acceptance|scope-audit|validation-plan|validate|record-validation|checkpoint-wave|resume-wave|build-review-bundle|check-progress|perf-report|cache-stats|cache-verify|cache-prune`. `tune-wave-plan` and planning are read-only and invoke no agents; `plan --wave-profile auto` evaluates deterministic batching profiles, charges repeated tests from one file at marginal complexity, and writes wave-tuning evidence. `run-wave` materializes one selected wave as an immutable bounded project-local workspace with fast/standard/audit execution policy and run context and refuses later waves until the predecessor has a valid acceptance receipt; `measure-wave` computes ready/draft/empty tests, blocking root TODOs, cascades, assertion preservation, and payoff candidates; `record-wave-decision` and `accept-wave` enforce the manager boundary; incremental commands compute changed-file impact, reuse only exact-input and exact-tool-contract PASS validation, checkpoint/resume work, and prepare reviewer evidence. `scope-audit` checks declared role access and evidence paths, `perf-report` aggregates wave/validation/agent timings, and cache commands inspect or safely prune unreferenced entries. In OpenCode, prefer `/supervised-task waves`.",

...
internal static class CliCommandCatalog
{
    ...
    static readonly CliCommandInfo[] Commands =
    {
        ...
        StableCommand("migration", "migration", false, false,
            ...
            "Direct command family: `selenium-pw-migrator migration inventory|cluster|tune-wave-plan|plan|plan show|run-wave`. `tune-wave-plan` and planning are read-only and invoke no agents; `plan --wave-profile auto` evaluates deterministic batching profiles, charges repeated tests from one file at marginal complexity, and writes wave-tuning evidence. `run-wave` materializes one selected wave as a bounded project-local workspace. In OpenCode, prefer `/supervised-task waves`.",

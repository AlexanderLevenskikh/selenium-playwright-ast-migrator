...
record ReportServeDashboardReport(DateTimeOffset GeneratedAtUtc, string Source, string ArtifactRoot, bool RecursiveArtifactLookup, MigrationBoardReport Current, ReportServeRunTrend[] Trends, ReportServeTodoCodeGroup[] TodoExplorer, ReportServeCountItem[] UnsupportedActions, ReportServeCountItem[] UnmappedTargets, ReportServeRootCause[] RootCauses, RuntimeFailureGroup[] RuntimeFailures, ReportServeTriageDecision[] TriageDecisions, MigrationIterationSnapshot IterationSnapshot, string[] MissingArtifacts, string[] StaticFiles, string? EvidenceZipPath);
record MigrationIterationSnapshot(string WorkspaceRoot, ProjectMemoryDashboardSnapshot Memory, StandardRunDashboardSnapshot StandardRun, ConfigMergeDashboardSnapshot ConfigMerge, string[] NextCommands, string[] EvidenceFiles);
record ProjectMemoryDashboardSnapshot(bool Exists, string SummaryPath, int Decisions, int Warnings, int Antipatterns, int FinalGateLessons, int ConfigDeltas, string DoctorCommand);
record StandardRunDashboardSnapshot(bool RunsExist, string LatestRunPath, int Runs, string LatestStatus, string VerificationStatus);
record ConfigMergeDashboardSnapshot(bool CandidateExists, bool ValidateReportExists, int Conflicts, string Status, string CandidatePath, string ValidateReportPath, string ConflictsPath);
...
record ReportServeRootCause(string Category, string GroupKey, string DisplayName, int Count, string SuggestedAction, string ExampleFile, int ExampleLine, string[] RepresentativeFiles);
record ReportServeTriageDecision(string Id, string ItemType, string Key, string RecommendedDecision, string Title, string Reason, int Count, string SuggestedAction, string ExampleFile, int ExampleLine, string[] EvidenceLinks, string TicketTemplate);
...
record PomFact(string SourceExpression, string OwnerType, string MemberName, string MemberKind, string Selector, string SelectorKind, string TargetKindSuggestion, string TargetExpressionSuggestion, string SourceFile, int SourceLine, string Confidence, bool RequiresReview, string FactOrigin, string Notes);
record PomUsageCandidate(string SourceExpression, string SuggestedTargetExpression, string SuggestedTargetKind, int Usages, string ExampleFile, int ExampleLine, string Confidence, bool RequiresSourceTruth, string Notes, string FactOrigin = "InferredUsage");
...
record MigrationRunbookReport(DateTimeOffset GeneratedAtUtc, string InputPath, string SourceFrontend, string SourceLanguage, string SourceFramework, string SourceConfidence, string TargetBackend, string TargetLanguage, string TargetFramework, string TargetTestFramework, GenerationPolicyReport GenerationPolicy, string[] ConfigLayers, RunbookProjectSummary ProjectSummary, RunbookPilotCandidate[] RecommendedPilotScope, RunbookCommandStep[] FirstCommandChain, RunbookRisk[] RiskMap, RunbookArtifact[] ArtifactsToCollect, string[] AcceptanceChecklist, string[] RecommendedNextActions);

...
function Test-AutoUpdatedKitOwnedFile([string]$RelativePath) {
    ...
    return (
        ...
        $normalized -eq "scripts/validate-installed-scripts.ps1" -or
        $normalized -eq "scripts/validate-installed-scripts.sh" -or
...
$quickStartLines = @(
    ...
    "",
    '```bash',
    "./$(Join-Path $Workspace 'scripts/apply-opencode-project-config.sh') --repo-root . --workspace `"$Workspace`"",
    '```',
    "",
    "This copies opencode.jsonc plus .opencode/agents and .opencode/commands into the repository root with backups.",
    "",
    "## 3. Give this prompt to your agent",
    "",
    '```text',

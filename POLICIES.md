...
Нельзя suppress-ить assertions через `SuppressedMethodPatterns`/`SuppressedMethods`: `Should`, `Assert`, `Expect`, `EqualTo`, проверки текста, значения, количества, видимости, URL и бизнес-данных должны стать Playwright/xUnit/NUnit assertion mapping или честным failing/manual TODO. Пустой зелёный тест после suppression считается критическим багом миграции.

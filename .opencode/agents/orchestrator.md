...
7. `REJECT_NO_PROGRESS` exhausts only the exact Core residual IDs bound to the candidate; `REJECT_REGRESSION` requires rollback but does not exhaust that residual. Another write requires a Core guard bound to a non-exhausted `--residual-id`; `ROLLBACK_CONFIRMED` must precede it after rejection. `REJECT_CYCLE` stops with `REMEDIATION_CYCLE_DETECTED`, but rollback still must be confirmed before handoff. The global no-progress streak is telemetry only; a plateau stop requires `REMEDIATION_RESIDUAL_CANDIDATES_EXHAUSTED`.

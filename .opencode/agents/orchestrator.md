...
4. For `REMEDIATE_CURRENT_WAVE`, send the selected highest-payoff root pattern to `migration-task-slicer`, execute one bounded normal implementation attempt, regenerate and validate the same wave, then run `migration record-wave-remediation` so the CLI derives `COMPLETED` versus `NO_PROGRESS`; re-measure. Implement cheap deterministic helper/POM behavior, but do not open an unbounded research loop.
5. For `SCAFFOLD_CURRENT_ROOT`, require measured `NO_PROGRESS` for the exact candidate, then add one narrow helper/POM scaffold rule. Preserve assignment/await shape, fail at runtime, respect scaffold limits, and regenerate. Never scaffold assertions, framework APIs, selectors, waits, or arbitrary unknown statements.
6. For `ACCEPT_WAVE`, `ACCEPT_WITH_SCAFFOLDING`, or `DEFER_SOFT_DEBT`, do not issue the receipt immediately. Run the metrics-bound final reviewer, final sentinel, and `migration scope-audit`; only then run `selenium-pw-migrator migration accept-wave --out migration/runs/<wave-id>`. The CLI refuses acceptance without the current hash-chained manager receipt, those final role receipts, and a fresh scope audit. Only the immutable acceptance receipt allows `migration run-wave` to materialize the next planned wave.

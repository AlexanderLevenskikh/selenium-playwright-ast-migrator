...
`/supervised-task` is the normal tester-facing entrypoint. It must work with no arguments. If the normalized base request is empty or only whitespace, inspect workspace state and choose the safe behavior below; do not ask the user what to do next and do not require them to know Harness internals.

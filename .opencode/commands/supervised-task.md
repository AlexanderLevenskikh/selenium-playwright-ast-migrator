...
7. Read `migration/plan/wave-tuning.md` and verify that the plan amortizes orchestration: only the smoke wave should be intentionally singleton, source files should not be fragmented without a hard-cap reason, and a project of roughly 80 tests should normally produce a single-digit or low-double-digit wave count rather than one wave per test.

...
## Continuous invocation mode (`continuous` / `--continuation auto`)

At the start of every invocation, parse `$ARGUMENTS` case-insensitively and normalize the continuation modifier before selecting the base mode:

- a standalone `continuous` token enables **continuous invocation mode**;
- the exact pair `--continuation auto` enables the same mode;
- remove only that modifier from a working copy of the arguments, then interpret the remaining text as the normal base request;
- an empty base request means ordinary state-aware `/supervised-task` resume in continuous mode;
- `continue`, `continue <topic-or-task>`, `waves`, `waves fresh`, and explicit bounded requests retain their normal meaning with continuous behavior layered on top;
- `sentinel`, `inspect`, and `qa` are intentionally one-shot forensic modes: do not loop them merely because a continuation modifier is present.

Supported examples are equivalent within each row:
...
Continuous intent is persisted for the active harness run so chat compaction, context compression, or a resumed OpenCode session cannot silently downgrade it to default mode. It does not weaken or permanently rewrite harness policy, execution profiles, role budgets, remediation budgets, scope, review, sentinel, validation, or final-gate rules. Clear it only after a real terminal condition, an explicit user `stop`/`pause` request, or a fresh/restarted run.

Before selecting any action, read `migration/state/harness-run.json`. When this invocation enables continuous mode, persist these fields in both the active state file and `migration/runs/<run-id>/run.json` when present: `continuationMode: "continuous"`, `continuousRequested: true`, `continuousSource: "supervised-task-arguments"`, and `stopOnlyOnTerminalCondition: true`. When those persisted fields already exist on a non-terminal active run, restore continuous behavior even when the current prompt is only `continue`, empty, or reconstructed after compaction. Never infer termination from repeated compressed summaries; state files are authoritative.

In continuous mode:

1. Keep executing the next runtime-authorized bounded action while state means `CONTINUE_REQUIRED`, `POST_FINAL_TASKS_READY`, `CURRENT_TICKET_ACTIVE`, `SAFE_CHECKPOINT`, a fresh successful `FINAL`/PASS checkpoint with more approved work, or persisted `FINAL_STOPPED_FOR_REVIEW`. If quality budget is blocked and no active agent-executable ticket remains, run the gate-followup slicer to materialize one instead of handing off with an empty backlog.
2. A fresh successful checkpoint is still recorded and reported in trace/evidence, but it is **not** a user-facing pause. Immediately re-read current ticket, continuation, quality budget, risk, recovery, and final-gate state and enter the same closed continuation loop that a later `/supervised-task continue` would have entered.
3. In wavefront mode, select/materialize the next pending planned wave only after the current wave has a valid immutable `wave-acceptance.json` bound to current generated output and metrics. Final/scope/quality-budget cleanliness without that receipt is insufficient. Do not skip measurement, `migration-wave-manager`, remediation, review, or acceptance to reach the next wave.
4. In ordinary non-wave mode, repeat the state-aware bounded lifecycle rather than stopping after the first successful ticket.
5. Do not recursively invoke `/supervised-task` or pretend a new chat command was issued. Continue inside the current orchestrator run by re-reading machine-readable state after every bounded action.
6. Treat “exactly one bounded action” as a **per-cycle safety boundary**, never as a per-invocation cap. After that action and its validation/gates, re-evaluate state. If another action is runtime-authorized, begin the next bounded cycle in the same invocation.
7. Emit concise checkpoint/heartbeat evidence between cycles when practical, but send a user-facing final handoff only on a terminal condition.

Continuous mode must stop on `DONE`, `FINAL_WITH_LIMITATIONS`, `WAVE_REMEDIATION_BUDGET_EXHAUSTED`, `HUMAN_DECISION_REQUIRED`, a `BLOCKED`/`BLOCKED_*` state with no runtime-authorized agent-executable remediation, critical risk, scope violation, denied permission, malformed or contradictory runtime/evidence, `NO_PROGRESS_DETECTED` after the allowed strategy-change budget, missing required user input, exhausted role/remediation/wall-clock/loop budget, or an explicit user stop. `BLOCKED_BY_WAVE_QUALITY_BUDGET` is not terminal while its report provides an actionable remediation `nextAction`, remediation budget remains, and the required writes stay inside allowed scope; it blocks the next wave, not the remediation loop. It is automatic continuation, not unbounded execution.

## Harness execution profile (`--execution-profile`)

Parse the execution-profile modifier independently from the continuation modifier and base request:

- `--execution-profile fast` selects the lightweight executor-first Harness route;
- `--execution-profile standard` selects executor plus reviewer, with watchdog/sentinel still triggered by risk or policy;
- `--execution-profile audit` selects the full Harness route with executor, reviewer, watchdog, and sentinel;
- remove the exact modifier pair from the working argument copy before interpreting the remaining base request;
- when no modifier is supplied and no immutable wave policy already exists, default to `fast`;
- an existing `migration/runs/<wave-id>/execution-policy.json` is immutable and authoritative. If the requested profile conflicts with that policy, do not rewrite it in place; stop with `EXECUTION_PROFILE_MISMATCH` and recommend a fresh wave/run with the requested profile.
...
/supervised-task waves fresh --execution-profile audit
```

Profile behavior:

| Profile | Intended use | Role routing |
|---|---|---|
| `fast` | Default/lightweight day-to-day migration | Executor first; reviewer/watchdog/sentinel are added only by runtime risk, policy, no-progress, or final-handoff requirements. |
| `standard` | Balanced migration when review should happen earlier | Executor and reviewer are expected; watchdog/sentinel remain conditional until final-handoff requirements apply. |
| `audit` | Full Harness for risky changes, releases, policy/runtime changes, or forensic validation | Executor, reviewer, watchdog, and sentinel are all required. |

All profiles keep immutable scope, validation, final reviewer, final sentinel, and final gate requirements. The profile changes routing cost, not safety guarantees.

## One-command wavefront start (`/supervised-task waves`)

After removing any continuation and execution-profile modifiers from the working argument copy, when the base request is exactly `waves`, `wave`, `wavefront`, `start waves`, or contains a clear request to start divide-and-conquer migration, enter **wavefront bootstrap mode**. When the arguments contain `waves fresh`, `fresh waves`, or `restart waves`, first enter **fresh-wavefront restart mode**. This mode is the preferred user-facing start path: the user should not need to run `kit init`, `bootstrap-opencode`, `kit doctor`, `migration plan`, or `migration run-wave` manually after installing/updating the tool.
...
1. Do **not** run `selenium-pw-migrator --mode migrate` against the full source project before a wave workspace exists. A full-source migrate from a fresh workspace is a protocol violation when `waves` was requested. After a wave is materialized, never point a direct full-project migrate at `migration/runs/wave-*/generated`; use that wave's `run-migrate.ps1` / `run-migrate.sh` and exact `selected-tests.txt`. Store exploratory full-project output only under a separate run-level location such as `migration/runs/<run-id>/full-project-rerun/generated`.
...
9. If no active wave run exists, materialize the first pending wave before implementation:
   `selenium-pw-migrator migration run-wave --plan migration/plan --wave wave-001 --workspace migration --out migration/runs/wave-001 --execution-profile <selected-profile>`
...
11. Create/resume the Harness run and continue through the normal lifecycle using the wave workspace as the active ticket. After a successful validation, run `migration checkpoint-wave`, then `migration build-review-bundle`; after interruption run `migration resume-wave` and follow its single `nextAction` instead of repeating discovery or migration. A checkpoint is not `DONE`, cached validation never replaces required gates, and the review bundle never replaces final reviewer/sentinel/final gate. After each bounded implementation/fix cycle run `selenium-pw-migrator migration check-progress --out migration/runs/<wave-id> --max-identical-snapshots 3`. `NO_PROGRESS_DETECTED` requires watchdog/strategy change instead of another retry. Before role dispatch, run `selenium-pw-migrator migration assess-agent-risk --out migration/runs/<wave-id>` and inspect the deterministic score/reasons. Then run `selenium-pw-migrator migration next-agent-action --out migration/runs/<wave-id>` and execute exactly the one emitted action. Before and after each dispatched role, use `migration record-agent-role` with matching role/phase and `STARTED`/`COMPLETED` evidence. Run `migration check-agent-budget` before automatic continuation. Do not choose the next role from prose alone, do not dispatch an already active role, and do not bypass `HUMAN_REVIEW_REQUIRED`. A `RUN_ROLE` decision is valid only while its `riskAssessmentFingerprint` still matches `agent-risk-assessment.json`; if evidence changes, rerun `next-agent-action`. `fast` may omit pre-execution roles, but final reviewer, final sentinel, scope checks, and final gate remain mandatory. Record material role file access with `migration record-role-scope-access` and run `migration scope-audit` before final handoff; actual out-of-scope access is never downgraded to a warning. All implementation writes remain under `migration/**` unless a later explicit policy grants more. For a long-running active role, renew its durable lease with `migration heartbeat-agent-role`; never fake a terminal event merely to unlock routing. After a process/session interruption, run `migration plan-agent-recovery` before dispatch. Follow `WAIT_FOR_ROLE`, execute the single `recover-agent-runtime` command only for `SAFE_REPAIR_AVAILABLE`, and stop on `BLOCKED`. Recovery may append a FAILED receipt for a stale role, rebuild a derived ledger head, archive an orphan lease, or quarantine incomplete atomic temp files; it must never rewrite a malformed hash-chained JSONL journal.
...
Compatibility wording: in the default non-continuous mode, a fresh `FINAL` checkpoint must **stop for review** once; a persisted `FINAL_STOPPED_FOR_REVIEW` checkpoint normally resumes the closed post-final development loop. In continuous mode, the fresh checkpoint is persisted but immediately consumed by that same closed loop without a user-facing pause. `FINAL_WITH_LIMITATIONS` / `WAVE_REMEDIATION_BUDGET_EXHAUSTED` is different: it is always a hard autonomous stop and must not resume post-final work without explicit user approval or `waves fresh`.
...
`/supervised-task` is the normal tester-facing entrypoint. It must work with no arguments. If `$ARGUMENTS` is empty, treat it as an empty normalized base request. If the normalized base request is empty or only whitespace, inspect workspace state and choose the safe behavior below; do not ask the user what to do next and do not require them to know Harness internals.
...
2. If the latest final gate is non-final and current-ticket, verify output, handoff, or continuation decision names an allowed next config/scaffold/evidence action under `migration/**`, execute exactly one next bounded action. If the named action is `slice-gate-followups`, run the slicer first, then route the generated `migration/current-ticket.md` through `migration-change-reviewer`. In continuous mode, complete the cycle, re-read state, and keep consuming authorized bounded actions until a real terminal condition; do not convert the end of one ticket into a final report.
2a. If no current ticket exists but `migration/plan/waves.json` exists, choose the next uncompleted wave as a bounded ticket. If `migration/runs/<wave-id>/input-scope.json` does not exist, first run `selenium-pw-migrator migration run-wave --plan migration/plan --wave <wave-id> --workspace migration --out migration/runs/<wave-id> --execution-profile <selected-profile>`. Before implementation, run `selenium-pw-migrator memory explain --workspace migration`, `selenium-pw-migrator memory doctor --workspace migration`, and `selenium-pw-migrator memory recall --file <file> --workspace migration` for every file in the wave/current-ticket scope. `memory recall` records receipts in `state/memory/recall-index.json` and `recall-ledger.jsonl`; final gate requires current-wave coverage when active memory exists. Treat the wave plan and run workspace as guidance, not permission to edit outside `migration/**`. Keep `config-delta.json` and `memory-delta.jsonl` wave-local until reviewed. When multiple reviewed deltas exist, run `selenium-pw-migrator config merge-deltas --base migration/adapter-config.json --deltas migration/state/memory/config-deltas --out migration/config-merge` and `selenium-pw-migrator config validate-merge --base migration/adapter-config.json --candidate migration/config-merge/adapter-config.merged.json --out migration/config-merge`; do not activate the candidate while `conflicts.jsonl` is non-empty.

2b. **Quality-manager boundary before the next wave.** After the current wave-local migration and deterministic validation, run `selenium-pw-migrator migration measure-wave --out migration/runs/<wave-id>`, then invoke `migration-wave-manager` with `wave-manager-packet.json`. Do not infer acceptance from `report.json`, syntax-fallback counters, TODO totals, or a green structural verify. The manager cannot override hard gates and must record exactly one decision through `migration record-wave-decision`.
   - `REMEDIATE_CURRENT_WAVE`: route the selected highest-payoff root pattern through `migration-task-slicer` and execute exactly one bounded normal implementation attempt against the same wave. Implement cheap deterministic helper/POM behavior, but stop before open-ended research; regenerate, validate, record remediation, and re-measure.
   - `SCAFFOLD_CURRENT_ROOT`: only after measured `NO_PROGRESS` for that exact helper/POM candidate, create one narrow runtime-failing scaffold rule, preserve assignment/await shape, and enforce scaffold-root/scaffold-only budgets. Never use scaffolding for assertions, framework APIs, selectors, waits, or generic unknown statements.
   - `ACCEPT_WAVE`, `ACCEPT_WITH_SCAFFOLDING`, or `DEFER_SOFT_DEBT`: run the metrics-bound final reviewer, final sentinel, and `migration scope-audit`, then run `migration accept-wave`; the CLI rejects a receipt without the current hash-chained migration-wave-manager receipt, those final role receipts, and a fresh scope audit. Only a valid `wave-acceptance.json`—an immutable receipt bound to current evidence—permits materializing the next wave.
   - `SPLIT_WAVE`: create a bounded plan-repair ticket; do not materialize a larger later wave.
   - `STOP_BUDGET_EXHAUSTED`: stop honestly as `DRAFT_WITH_DEBT` / `FINAL_WITH_LIMITATIONS`.
   - `REQUEST_HUMAN_DECISION`: stop only for product semantics, forbidden writes, credentials, or real competing priorities.
   `fast` changes role/validation cost, never the acceptance invariants. Never edit source reports, metrics, continuation state, or ticket status to obtain acceptance.

3. If the latest final gate has just produced a fresh `FINAL` / `HARNESS_CONTINUATION_FINAL` checkpoint in the current supervised-task run and continuous mode is not active, stop for review once: summarize the completed checkpoint, final-gate evidence, changed artifacts, remaining risks, and one recommended continue command. If `migration/state/harness-run.json` already contains `FINAL_STOPPED_FOR_REVIEW` when this command starts, this is no longer a fresh checkpoint; resume the closed post-final loop from step 1a instead. Explicitly say only for the fresh checkpoint: "I stopped because the SUCCESS checkpoint requires review before post-final research or another bounded ticket." Do not show a broad menu.
In default mode, do not show a broad menu after a successful checkpoint; give one recommended continue command instead: `/supervised-task continue`. In continuous mode, do not emit that pause command: record the checkpoint and immediately continue through the closed loop.
4. If the normalized base request explicitly requests `continue` after a `FINAL` / `FINAL_STOPPED_FOR_REVIEW` checkpoint, use the same low-prompt closed loop as step 1a. Explicit `continue` is still supported, but it is no longer required once the persisted run status is `FINAL_STOPPED_FOR_REVIEW`; zero-argument `/supervised-task` must also resume that loop. If the user names a specific test/helper/TODO, pass that as the research topic. If the user names a concrete implementation task, first require approved research/task slicing when research artifacts exist; otherwise start the smallest safe bounded implementation ticket from the recommended remaining risks.
5. If `continuation-decision.json` contains bounded auto-continuation for the exact next action, obey that budget. Otherwise a fresh `FINAL` checkpoint stops once for review in default mode; continuous mode consumes it immediately. A persisted `FINAL_STOPPED_FOR_REVIEW` status always resumes the closed post-final loop; plain `continue`, zero-argument `/supervised-task`, and their continuous forms all mean post-final research/review/task slicing first.
...
7. In default mode, stop for a fresh `FINAL` stop-for-review. In continuous mode, consume that checkpoint and continue. In every mode stop for `FINAL_WITH_LIMITATIONS` / `WAVE_REMEDIATION_BUDGET_EXHAUSTED`, `DONE`, `HUMAN_DECISION_REQUIRED`, an explicit `BLOCKED_*` state with no runtime-authorized agent-executable remediation, critical risk, scope violation, missing required user input, denied writes, malformed/contradictory state, loop-guard/no-progress block, or autonomous budget/plateau limits. `BLOCKED_BY_WAVE_QUALITY_BUDGET` is excluded while actionable in-scope remediation and budget remain. Persisted `FINAL_STOPPED_FOR_REVIEW` is not a stop condition; it is the closed-loop dispatch state.
Continuous-mode cycle rule: every executor dispatch remains one bounded ticket followed by validation, review, sentinel/scope/harness checks, and final gate. Continuous mode repeats this guarded cycle; it never turns several tickets or waves into one unreviewed batch.
...
When the persisted active run is `FINAL_STOPPED_FOR_REVIEW`, prefer the closed post-final loop over immediate implementation whether the normalized base request is empty or explicitly says `continue`, unless the user names a concrete implementation task. Use this priority order:
...
9. After task slicing and change-review approval, delegate exactly one selected ticket to `executor` under `migration/**` for the current bounded cycle. If `boundedAutoContinuation.allowed` is present, obey its budget; if it is absent, default mode grants only this one bounded executor task. Continuous mode may begin another bounded cycle only after this ticket passes its required reviewer/scope/harness/validation/final-gate path and refreshed state authorizes another action. Stop when reviewer/policy or any terminal condition blocks the ticket.
...
1. If `migration/current-ticket.md` exists and `migration/state/continuation-decision.json` says `POST_FINAL_TASKS_READY`, route the current ticket through `migration-change-reviewer`, then delegate exactly one bounded executor task for this cycle. The persisted `FINAL_STOPPED_FOR_REVIEW` state is sufficient consent for that task under `migration/**`; in continuous mode, a later task still requires a fresh post-gate state decision.
...
14. Stop after at most 2 fix-review cycles for one bounded action. Continuous mode may advance to another runtime-authorized bounded action, but it does not reset no-progress, role, remediation, or safety budgets.
...
18. If continuation status is `CONTINUE_REQUIRED`, do not send a user-facing handoff yet. Execute exactly one next bounded action named by the decision/current ticket/handoff, then rerun scope, harness policy, verification, and final gate. A response that only repeats NOT FINAL / NOT RUNTIME READY while `CONTINUE_REQUIRED` exists is a protocol violation. In continuous mode, repeat this step after every completed bounded cycle until a terminal condition is persisted.
19. After every fresh successful `FINAL` / PASS checkpoint produced in the current run, stop once and report only in default mode. In continuous mode, persist the checkpoint, re-read machine-readable state, and immediately resume the closed post-final loop or next eligible wave. On any later `/supervised-task` invocation where `harness-run.json` is already `FINAL_STOPPED_FOR_REVIEW`, resume the closed post-final loop automatically; starting implementation still requires approved research, task slicing into `migration/current-ticket.md`, change-review approval, or bounded auto-continuation in `continuation-decision.json`.
19a. **Continuous handoff veto:** immediately before any user-facing final report, re-read `continuation-decision.json`, `task-slice-result.json`, `current-ticket-status.json`, `current-ticket.md`, `wave-quality-budget.json`, and the active backlog. In continuous mode, handoff is forbidden while any of these is true: status is `CONTINUE_REQUIRED`, `POST_FINAL_TASKS_READY`, or `CURRENT_TICKET_ACTIVE`; `mustContinueBeforeUserMessage` is `true`; a selected/approved `AGENT_EXECUTABLE` ticket is `READY`, `IN_PROGRESS`, or `REVIEW_READY`; or `BLOCKED_BY_WAVE_QUALITY_BUDGET` has an actionable remediation `nextAction` and remaining budget. Execute the next bounded cycle instead. Never emit `To continue, run: /supervised-task continue` in those states.
19b. **Empty-backlog remediation dispatch:** completed tickets do not make a blocked quality budget terminal. When the active backlog has no executable non-terminal ticket but `wave-quality-budget.json` is blocked with remaining remediation budget, run `migration/scripts/slice-gate-followups.ps1 -Workspace migration` (or `.sh`), require it to refresh `current-ticket.md`, `current-ticket-status.json`, `task-slice-result.json`, and continuation state, then dispatch the new bounded ticket. A prose suggestion such as “consider creating another mapping ticket” is not a valid next action.
19c. **Scope split rule:** reading Selenium POM/source files is allowed; modifying them is not. Creating or extending target-side Playwright page objects, local scaffolds, config, generated output, or proposals under `migration/**` is `AGENT_EXECUTABLE`. If a candidate mixes an allowed migration-local implementation with an out-of-scope product edit, split it into separate tickets and execute the migration-local ticket instead of classifying the whole item `BLOCKED_BY_SCOPE`.
20. Final report:
   - active run id;
   - changed files;
   - verification result;
   ...
   - trace/events status;
   - remaining risks;
   - in default mode, one recommended continue command in the form `To continue, run: /supervised-task continue`; in continuous mode, omit a continue command because handoff occurs only at a terminal stop;
   - anything intentionally not fixed.

After a SUCCESS checkpoint report in default mode, never leave the user guessing why no work was done: state that the checkpoint is complete, `harness-run.json` should be `FINAL_STOPPED_FOR_REVIEW` when present, and provide exactly one command: `To continue, run: /supervised-task continue`. In continuous mode, do not stop at that checkpoint; continue until a terminal condition and report the terminal reason.

Continuation rule: Continue with the next bounded action when `migration/state/continuation-decision.json` reports `CONTINUE_REQUIRED`, the persisted active run is `FINAL_STOPPED_FOR_REVIEW`, the user explicitly requests plain `continue`, continuous mode is active, or bounded auto-continuation allows this exact action. Continuous mode additionally consumes fresh successful checkpoints and eligible next waves without a user-facing pause, while all terminal stop conditions and budgets remain authoritative. A `Developer action` in research is not a terminal human handoff until `migration-task-slicer` classifies it as non-agent-executable.
...
Wave quality budget rule: after any `runs/wave-*` execution, run `migration/scripts/evaluate-wave-quality-budget.ps1` / `.sh` before selecting another wave. If the report says `BLOCKED_BY_WAVE_QUALITY_BUDGET`, do not start a new wave; route to mapping/research memory and one bounded config/POM/recognizer improvement ticket. In continuous mode this status blocks wave advancement only: while remediation budget and an actionable in-scope `nextAction` remain, complete that ticket, rerun the budget/gates, and continue with the next authorized remediation cycle rather than handing off.

## Mapping/research memory loop

When `evaluate-wave-quality-budget` reports `BLOCKED_BY_WAVE_QUALITY_BUDGET`, do not select another wave. If `scopeIntegrity.status` is `CONTAMINATED_BY_FULL_SCOPE_RERUN`, first preserve the full-project output under `migration/runs/<run-id>/full-project-rerun/**`, restore the exact wave-local generated scope, and rerun the wave wrapper. Otherwise run `migration/scripts/collect-mapping-research-memory.ps1` / `.sh` first. Use `mapping-research-memory/v1`, `state/mapping-research-memory.json`, and `state/mapping-research-candidates.jsonl` to route one bounded config/POM/recognizer or verify-harness improvement ticket. After that ticket completes, re-evaluate quality budget and continuation state. In continuous mode, if no active remediation ticket exists, automatically run `slice-gate-followups` to create one; select the next approved remediation ticket in the same invocation and stop only on exhausted budget, no-progress, a concrete blocker, or another terminal condition.
...
Performance/cache operations: use `migration perf-report --out <run-dir>` for the correlated breakdown, `migration cache-stats|cache-verify --workspace migration` for inspection, and run `cache-prune` as dry-run before `--cache-apply true`. Never remove cache entries referenced by active runs manually.

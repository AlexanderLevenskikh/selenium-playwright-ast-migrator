...
9. After a non-final final gate, read `state/continuation-decision.json`; if it says `CONTINUE_REQUIRED`, execute exactly one next bounded action before any user-facing handoff. In continuous mode, that is one cycle boundary, not an invocation cap: rerun validation/gates, re-read state, and immediately execute the next authorized bounded cycle until a terminal condition is persisted.
...
10. After a successful FINAL/PASS checkpoint, stop and report in default mode. If the initiating `/supervised-task` invocation explicitly used `continuous` or `--continuation auto`, persist `continuationMode: continuous`, `continuousRequested: true`, and `stopOnlyOnTerminalCondition: true` in the active `harness-run.json`/`run.json`, persist the checkpoint, and keep executing consecutive runtime-authorized bounded cycles. A later compacted or resumed session must restore this mode from state until a terminal condition, explicit stop/pause, or fresh run clears it. After every cycle, rerun required checks and re-read machine-readable state. Do not hand off merely because one ticket finished. Do not start another run or ticket unless explicit continue/continuous intent or `state/continuation-decision.json` grants the exact next action; all blockers, risk, scope, review, sentinel, final-gate and budget rules remain mandatory.

10a. Continuous intent is persisted for the active run and restored after compaction or session resume. It may apply to ordinary resume, a bounded request, `continue`, `waves`, or `waves fresh`; it is cleared by a real terminal condition, explicit stop/pause, or fresh/restarted run. It never applies as a loop to one-shot `sentinel`/`inspect`/`qa`, never recursively invokes a slash command, and never turns multiple tickets/waves into one unreviewed batch.
...
- Resolve role routing with `migration next-agent-action --out migration/runs/<wave-id>` and execute exactly one emitted action. Record every dispatched role through `migration record-agent-role` with `STARTED` and terminal evidence; never hand-edit `agent-role-events.jsonl` or reuse a stale role receipt. `migration check-agent-budget` is mandatory before automatic continuation. Fast routing may omit pre-execution roles, but final reviewer, final sentinel, scope/harness checks, and final gate remain mandatory. Record material role reads/writes/discovery through `migration record-role-scope-access`; run `migration scope-audit` before final handoff. Declared out-of-scope access is always a failure.
- After validation and review-bundle materialization, run `migration measure-wave --out migration/runs/<wave-id>` before final reviewer/sentinel work. The resulting `wave-quality-metrics.json` and `wave-manager-packet.json` are deterministic boundary evidence derived from the generated tree; legacy semantic/fallback/TODO/unmapped counters remain visible diagnostics but are not acceptance authority.
- Dispatch `migration-wave-manager` exactly once per measured boundary. It may choose only `ACCEPT_WAVE`, `ACCEPT_WITH_SCAFFOLDING`, `REMEDIATE_CURRENT_WAVE`, `SCAFFOLD_CURRENT_ROOT`, `SPLIT_WAVE`, `DEFER_SOFT_DEBT`, `STOP_BUDGET_EXHAUSTED`, or `REQUEST_HUMAN_DECISION`. It cannot override missing tests, unexpected out-of-wave tests, empty/comment-only tests, blocking root TODOs, lost assertions, missing active behavior, active suppression/placeholders, stale validation, scope failure, or tampered evidence.
- `REMEDIATE_CURRENT_WAVE` permits exactly one highest-payoff normal implementation attempt. Migrate cheap deterministic helper/POM behavior, but do not start open-ended product research. `SCAFFOLD_CURRENT_ROOT` is permitted only after measured `NO_PROGRESS` for the exact helper/POM candidate and may add only a narrow explicit runtime-failing scaffold. Assertions, framework APIs, selectors, waits, generic unknown statements, catch-all patterns, and silent default-return stubs are forbidden. Scaffold-root and scaffold-only-test budgets prevent scaffolding from becoming the whole migration. Both paths are followed by regeneration and executed validation of the same wave, then `migration record-wave-remediation`. The CLI derives `COMPLETED` versus `NO_PROGRESS` from before/after evidence; agent-declared success is not authority. Re-measure afterward. `fast` changes ceremony and cycle budget, not hard quality thresholds. Exhausted/no-progress budget produces an honest `DRAFT_WITH_DEBT`/`FINAL_WITH_LIMITATIONS`, never a fabricated PASS.
- Final reviewer and final sentinel are paid only after the manager proposes `ACCEPT_WAVE`, `ACCEPT_WITH_SCAFFOLDING`, or `DEFER_SOFT_DEBT`. Then rerun scope audit and issue `migration accept-wave`. Acceptance requires a current hash-chained `migration-wave-manager/quality COMPLETED` receipt plus metrics-bound final reviewer and sentinel receipts; a decision JSON alone is insufficient. Only a current immutable `wave-acceptance.json` permits materializing a later wave. The receipt is invalidated by generated-code drift, stale metrics/decision, malformed remediation history, broken role evidence, or changed scope evidence; final quality evaluation revalidates the complete materialized wave chain.
...
16a. Continuous-mode handoff is forbidden while `mustContinueBeforeUserMessage` is true, continuation status is `CONTINUE_REQUIRED`/`POST_FINAL_TASKS_READY`/`CURRENT_TICKET_ACTIVE`, a selected agent-executable current ticket is non-terminal, or wave quality budget exposes an actionable remediation `nextAction` with remaining budget. If the backlog contains only completed/blocked tickets while quality remediation remains, run `slice-gate-followups` to materialize the next ticket; backlog exhaustion is not terminal by itself.
...
Wave quality budget is mandatory for wave runs: after materializing or executing `runs/wave-*`, run `migration/scripts/evaluate-wave-quality-budget.ps1` / `.sh` before the next wave. Direct full-project migration output must never be written into a materialized `runs/wave-*/generated` directory; use the wave wrapper and selected-tests manifest, and keep exploratory full-project output under `runs/<run-id>/full-project-rerun/**`. If it writes `BLOCKED_BY_WAVE_QUALITY_BUDGET`, do not start another wave; route into mapping/research memory and one bounded config/POM/recognizer improvement ticket. This status is not a terminal continuous-mode handoff while an actionable in-scope remediation `nextAction` and budget remain.

## Mapping/research memory loop

When `evaluate-wave-quality-budget` reports `BLOCKED_BY_WAVE_QUALITY_BUDGET`, do not start another wave. If `scopeIntegrity.status` is `CONTAMINATED_BY_FULL_SCOPE_RERUN`, preserve the full-project draft, restore exact wave-local evidence, and rerun the wave wrapper first. Otherwise run `migration/scripts/collect-mapping-research-memory.ps1` / `.sh`. It writes `mapping-research-memory/v1`, `state/mapping-research-memory.json`, `state/mapping-research-memory.md`, `state/mapping-research-candidates.jsonl`, and `runs/<run-id>/research/mapping-research-memory.*`. Use those artifacts to slice exactly one bounded config/POM/recognizer or verify-harness improvement ticket. In continuous mode, after that ticket and its gates, re-evaluate state and consume the next approved remediation ticket instead of issuing a final report.
...
- Production wavefront plans begin with a one-test `smoke-validation` wave, then a bounded representative `mixed-calibration` wave selected with `--representatives-per-cluster`, and only then affinity-packed scale waves. Smoke proves mechanics; calibration must produce reusable pattern learning and a valid acceptance receipt before scaling. Repeated tests from the same file use marginal complexity so role overhead is amortized instead of producing one wave per test.
...
- Use `migration perf-report` for one correlated wave/validation/agent breakdown. Use `migration cache-stats`, `cache-verify`, and dry-run `cache-prune` for cache operations; never delete active-run referenced entries manually.

## Source/POM scope split

Reading Selenium source and POM files is allowed evidence gathering. Modifying those product files is forbidden in artifact-only mode. Creating target-side Playwright page objects, local scaffolds, generated output, adapter config, or proposals under `migration/**` is agent-executable. Mixed candidates must be split into an executable migration-local ticket and a separately recorded out-of-scope product edit; never block both by classifying the whole candidate `BLOCKED_BY_SCOPE`.

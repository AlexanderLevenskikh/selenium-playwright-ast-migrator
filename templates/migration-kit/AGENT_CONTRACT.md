...
10. After a successful FINAL/PASS checkpoint, stop and report in default mode. If the initiating `/supervised-task` invocation explicitly used `continuous` or `--continuation auto`, persist the checkpoint and keep executing consecutive runtime-authorized bounded cycles in the same invocation. After every cycle, rerun required checks and re-read machine-readable state. Do not hand off merely because one ticket finished. Do not start another run or ticket unless explicit continue/continuous intent or `state/continuation-decision.json` grants the exact next action; all blockers, risk, scope, review, sentinel, final-gate and budget rules remain mandatory.

10a. Continuous intent is invocation-local. It may apply to ordinary resume, a bounded request, `continue`, `waves`, or `waves fresh`; it never applies as a loop to one-shot `sentinel`/`inspect`/`qa`, never recursively invokes a slash command, and never turns multiple tickets/waves into one unreviewed batch.

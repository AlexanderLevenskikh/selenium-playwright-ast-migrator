...
- Keep `migration/runs/<wave-id>/wave-manifest.json`, `execution-policy.json`, `run-context.json`, `input-scope.json`, `wave-validation.json`, `validation-plan.json`, `validation-result.json`, `latest-checkpoint.json`, `resume-decision.json`, `review/review-bundle.json`, `performance-trace.json`, `run-summary.md`, and `wave-status.json` as evidence for the bounded wave.
- After generated output changes, run `migration validation-plan`; execute the recommended checks that satisfy current project gates, then record the real command and exit code with `migration record-validation`. A reusable PASS without command evidence is invalid; failed/stale validation is never cached.

...
Continuous mode is scoped to the current `/supervised-task` invocation. It does not weaken or permanently rewrite harness policy, execution profiles, role budgets, remediation budgets, scope, review, sentinel, validation, or final-gate rules.

In continuous mode:

1. Keep executing the next runtime-authorized bounded action while state means `CONTINUE_REQUIRED`, `SAFE_CHECKPOINT`, a fresh successful `FINAL`/PASS checkpoint with more approved work, or persisted `FINAL_STOPPED_FOR_REVIEW`.

...
1. If `migration/current-ticket.md` or `migration/state/backlog/post-final-tasks.jsonl` already exists, do not re-run broad research first. Route the selected ticket through `migration-change-reviewer`, then delegate exactly one bounded executor task.

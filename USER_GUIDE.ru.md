...
Сначала установите CLI. Рекомендуемый standalone-установщик не требует .NET SDK или .NET Runtime; см. [Standalone installation](docs/standalone-installation.ru.md). Затем используйте короткий путь для нового пользователя:
...
Используйте standalone, если нужно только запускать CLI и не хочется ставить .NET SDK или .NET Runtime:

...
Обычный запуск создаётся напрямую командой `selenium-pw-migrator run`; агент не должен придумывать или реконструировать validation artifacts.
...
Объединяет проверенные run-local файлы `config-delta.json` в candidate config и валидирует результат до promotion. Это безопасный мост между обычными запусками с evidence и основным `adapter-config.json`.
...
Обычный запуск считается завершённым только при наличии согласованных реальных artifacts. Final gate никогда не реконструирует отсутствующий результат вручную.

### Snapshot обычного запуска / memory / config merge

Начинайте review с dashboard:

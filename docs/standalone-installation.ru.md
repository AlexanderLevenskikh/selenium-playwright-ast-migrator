...
Installer теперь **переставляет** standalone-папку в начало `PATH`, даже если она уже была добавлена раньше, но стояла после `.dotnet\tools`. Если нужно заодно удалить старую global dotnet tool установку, запусти Windows installer с `-RemoveDotnetTool`:

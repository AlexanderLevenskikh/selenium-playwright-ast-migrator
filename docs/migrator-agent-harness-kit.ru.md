...
Затем выполняются установленные scope-, artifact- и final-gate-проверки. Необязательная команда `/supervised-task` запускает ту же последовательность и может выполнить до пяти циклов с одним ограниченным исправлением, полностью повторяя source scope после каждого цикла.

...
| `/supervised-task <bounded request>` | свободный текст | Передать supervisor конкретную ограниченную задачу | Сохраняет state-aware dispatch, scope, review, risk, recovery и final-gate правила. Не обходит активный blocker или current ticket. |

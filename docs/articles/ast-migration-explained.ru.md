...
Эта модель actions — мост между Selenium-синтаксисом и Playwright-рендерингом.
...
2. `examples/teaching-demo/adapter-config.json` связывает source expression с
   target locator:
...
## Шаг 5: сохранять неопределённость, а не маскировать её
...
Это не обычные UI actions. Они могут переехать в Playwright fixture, navigation
helper, wait policy или target PageObject constructor. Если инструмент не может
доказать правильное поведение в target-коде, он должен сохранить source как
TODO/source comment, а не генерировать красивый, но выдуманный код.

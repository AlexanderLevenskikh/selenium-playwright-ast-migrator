...
After a FINAL/PASS checkpoint, `/supervised-task` stops for review and prints one recommended `/supervised-task continue ...` command. Run that explicit continue command to start exactly one next bounded ticket.
